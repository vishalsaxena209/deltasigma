%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% Finding MSA and peak SNR using simulation
% Slow ramp input (method by Lars Risbo).
% Vishal Saxena, BSU
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
clear all; clc; close all;

%% User params
order = 3;  % Order of the NTF
OSR = 64;   % OSR
...
opt = 0;    % Optimize NTF zeros for opt=1
...
hold on; grid on;
plot(inst_index, u(inst_index), 'ro');
xlabel('n'); ylabel('input amplitude, u');
xlim([0 length(u)]);

title('MSA Estimation by Simulation');
subplot(2,1,2)
plot(log10(abs(y)), 'r'); 
hold on; grid on;
plot(inst_index, log10(abs(y(inst_index))), 'bo');
...
figure()
plot(u/(nLev-1), log10(abs(y)), 'b');
hold on; grid on;
plot(u(inst_index)/(nLev-1), log10(abs(y(inst_index))), 'ro');
ylabel('log |y|'); xlabel('Input, u');
xlim([0 1]);
text(0.5,5,s);
title('MSA Estimation by Simulation');
